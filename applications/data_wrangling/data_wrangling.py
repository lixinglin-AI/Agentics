...
from utils import crew_prompt_params, custom_instruction, get_header_csv_file
...
async def main(args):
    ...
    dataset = await dataset.self_transduction(
        header[:-1],
        [header[-1]],
        instructions=custom_instruction.format(target_field=args.impute_col),
    )
    return dataset
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--csv-file", type=str, default="buy_test_with_fewshots__k=1.csv"
    )
    parser.add_argument("--impute-col", type=str, default="manufacturer")
    args = parser.parse_args()
    data_path = os.fspath(
        Path(__file__).resolve().parent.parent.parent / "data" / "data_wrangling"
    )
